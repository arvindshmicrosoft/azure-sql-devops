-- Setup sample data only if table is empty
if NOT EXISTS (SELECT * FROM dbo.TrainingSession)
BEGIN
...
if user_id('DotNetWebApp') is null begin
    create user DotNetWebApp with password = '$(SQLPassword)';
    grant connect to DotNetWebApp;
end
go
